...
// @require      https://raw.githubusercontent.com/diamondplus89/labassiSTMOD/main/anchor-browserified.js
// @require      https://raw.githubusercontent.com/diamondplus89/labassiSTMOD/main/buffer-browserified.js
// @require      https://raw.githubusercontent.com/diamondplus89/labassiSTMOD/main/bs58-browserified.js
...
*/				
	...
	function calcWarpFuelReq(fleet, startCoords, endCoords) {
		if(CoordsEqual(startCoords, endCoords)) {
			cLog(3, `${FleetTimeStamp(fleet.label)} calcWarpFuelReq: Same coords`, startCoords, endCoords);
			return 0;
		}
		...
		if(!startX || !startY || !endX || !endY) {
			cLog(3, `${FleetTimeStamp(fleet.label)} calcWarpFuelReq: Bad coords`, startCoords, endCoords);
			return 0;
		}
...
*/
	...
	async function saveAssistInput() {
		...
		for (let [i, row] of fleetRows.entries()) {
			...
			if(fleetAssignment !== '') {
				...
				console.log('saveAssistInput: warpCost + coords', warpCost, starbaseCoords, destCoords);
	...
	async function handleMovement(i, moveDist, moveX, moveY) {
		...
		if (fleetState == 'Idle' && extra.length > 1 && moveDist && moveX !== null && moveX !== '' && moveY != null && moveY !== '') {
			console.log(extra, moveX, moveY);
			if (extra[0] !== moveX || extra[1] !== moveY) {
				const oldWarpCost = calculateWarpFuelBurn(userFleets[i], moveDist);
				let warpCost = calcWarpFuelReq(userFleets[i], extra, [moveX, moveY]);
				cLog(3, `${FleetTimeStamp(userFleets[i].label)} handleMovement:`, extra, [moveX, moveY]);
				cLog(3, `${FleetTimeStamp(userFleets[i].label)} oldWarpCost: ${oldWarpCost}`);
				cLog(3, `${FleetTimeStamp(userFleets[i].label)} warpCost: ${warpCost}`);
					...
					if ((fleetCoords[0] !== destCoords[0] || fleetCoords[1] !== destCoords[1])) {
						if (!userFleets[i].state.includes('Warp C/D')) {
							...
							if (userFleets[i].moveType == 'warp') {
									const oldFuelNeeded = calculateWarpFuelBurn(userFleets[i], distToStarbase) + calculateWarpFuelBurn(userFleets[i], 2);
									fuelNeeded = 
										calcWarpFuelReq(userFleets[i], fleetCoords, [starbaseX, starbaseY]) + 
										calcWarpFuelReq(userFleets[i], fleetCoords, destCoords);
									cLog(3, `${FleetTimeStamp(userFleets[i].label)} handleScan:`, fleetCoords, [starbaseX, starbaseY], destCoords);
									cLog(3, `${FleetTimeStamp(userFleets[i].label)} oldWarpCost: ${oldFuelNeeded}`);
									cLog(3, `${FleetTimeStamp(userFleets[i].label)} warpCost: ${fuelNeeded}`);
							} else {
	...
	async function handleMining(i, fleetState, fleetCoords, fleetMining) {
		...
		cLog(3, `${FleetTimeStamp(userFleets[i].label)} handleMining -> calcWarpFuelReq:`, fleetCoords, [starbaseX, starbaseY], [destX, destY]);
		const warpCostToTarget = calcWarpFuelReq(userFleets[i], fleetCoords, [destX, destY]);
	...
	async function getFleetFuelData(fleet, currentPos, targetPos) {
		...
		cLog(3, `${FleetTimeStamp(fleet.label)} getFleetFuelData -> calcWarpFuelReq:`, currentPos, targetPos);
		return {
			account,
			token,
			amount,
		...
		try { 
			...
			if(fleetParsedData.assignment) {
				...
				if(!fleet.initilizedScanPDAs && fleetParsedData.assignment == 'Scan') {
					cLog(1,`${FleetTimeStamp(fleet.label)} Initilizing Scan PDAs`);
					await getAccountInfo(fleet.label, 'fleet SDU token', fleet.sduToken) || await createPDA(fleet.sduToken, fleet.account.cargoHold, SDUPK, fleet);
